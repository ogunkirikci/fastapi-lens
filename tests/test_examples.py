...
def test_dependency_example_runs_and_exposes_dashboard() -> None:
    from examples.dependencies.app import app, profiler

    with TestClient(app) as client:
        response = client.get("/profile")
        dashboard = client.get("/__latensight__/")
...
def test_sqlalchemy_example_runs_without_persisting_local_data() -> None:
    ...
    example.profiler.uninstrument_sqlalchemy(original_engine)
    ...
    example.profiler.instrument_sqlalchemy(example.engine)

    with TestClient(example.app) as client:
        response = client.get("/items/1")
        dashboard_api = client.get("/__latensight__/api/traces")
